...
import com.suraj.cook_it.databinding.FragmentThirdBinding
...
class ThirdFragment : Fragment() {
    ...
    private fun initData() {
        recipeViewModel.recipeListViewPager.observe(viewLifecycleOwner) {
            binding.txtFirstFragment.text = it?.get(2)?.title
            Picasso.get().load(it?.get(2)?.imageUrl).into(binding.imgFirstFragment)
            recipe = it?.get(2)
        }
    }
